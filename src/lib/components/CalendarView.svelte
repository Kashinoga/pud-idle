...
<div class="calendar-view card">
  ...
  <div class="time-display">
    ...
    <div class="status"><span class={$running ? 'running' : 'paused'}>{$running ? 'Running' : 'Paused'}</span> <span class="speed">×{$speed}</span></div>
  </div>
...
<style>
  ...
  .date { color: var(--muted); font-size: small; }
  .time { font-size: xx-large; font-weight: 700; color: var(--foreground); letter-spacing: 0.02em; }
  .status { display: flex; gap: var(--space-xs); align-items: baseline; }
  .status .running { color: #22c55e; }
  .status .paused { color: #ef4444; }
  .status .speed { color: var(--primary); font-weight: 600; }
  .controls { display: flex; flex-direction: column; gap: var(--space-4xs); }
  .run-group, .speed-group { display: flex; flex-wrap: wrap; gap: var(--space-4xs); }
</style>
